"""
MaxLFQ-like quantification utilities.

This module collects functions adapted from the AlphaPept / MaxLFQ code base:
- Gaussian/elution profile simulation for peptides
- Experiment-wide normalization (SLSQP, BFGS)
- MaxLFQ-style protein LFQ profiles (single-threaded and parallel)
- Convenience wrappers for taxon-level LFQ

All original function bodies are kept unchanged for traceability; only
additional comments and docstrings have been added around them.
"""
...
# ---------------------------------------------------------------------------
# 1. Simulation utilities: Gaussian profiles and synthetic sample data
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2. Experiment-wide normalization (SLSQP / BFGS) and delayed normalization
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. Dummy data for benchmarking and testing
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4. Core MaxLFQ math: peptide ratios and triangle error
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. Solvers for protein profiles (L-BFGS-B / SLSQP / Powell / trf)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. Protein-level LFQ: single-protein profile and parallel wrappers
# ---------------------------------------------------------------------------
...
def protein_profile_parallel(df: pd.DataFrame, minimum_ratios: int, field: str, callback=None) -> pd.DataFrame:
    ...
    if len(samples) > 1:
        ...
        #     results = p.map(
        #         lambda chunk: protein_profile(samples, minimum_ratios, chunk),
        #         split_df
        #     )
...
# ---------------------------------------------------------------------------
# 7. Taxon-level LFQ wrapper using the same MaxLFQ machinery
# ---------------------------------------------------------------------------
from itertools import combinations
from numba.typed import List

def lfq_by_taxon(peptide_run_df, peptide_to_taxon, min_ratios=2):
    """
    Derive LFQ-like intensities per taxon by reusing the MaxLFQ protein
    machinery (get_protein_ratios + solve_profile).

    Parameters
    ----------
    peptide_run_df : pd.DataFrame
        Peptide × run matrix of intensities. Index = peptide IDs, columns = runs.
    peptide_to_taxon : dict
        Mapping from peptide ID -> taxon label (e.g. organism or clade).
    min_ratios : int, optional
        Minimum number of peptide ratios required to consider a taxon for LFQ.

    Returns
    -------
    pd.DataFrame
        Index = taxa, columns = "<run>_LFQ", with LFQ-like intensities per taxon.
        Taxa for which no stable profile can be obtained are filled with NaN.
    """
    taxa = sorted(set(peptide_to_taxon.values()))
    runs = peptide_run_df.columns.tolist()
    result = pd.DataFrame(index=taxa, columns=[r+'_LFQ' for r in runs])

    # pre-build all column pairs once
    combs = List()
    [combs.append(c) for c in combinations(range(len(runs)), 2)]

    for tax in taxa:
        sub = peptide_run_df.loc[
            [p for p,t in peptide_to_taxon.items() if t == tax]
        ].replace(0, np.nan)

        # functions borrowed from AlphaPept's MaxLFQ
        ratios = get_protein_ratios(sub.values, combs, min_ratios)
        I_hat, ok = solve_profile(ratios, 'L-BFGS-B')

        if ok and np.isfinite(I_hat).any():
            # rescale by total peptide intensity of that taxon
            scale = sub.sum(axis=0).values
            lfq   = I_hat * scale.sum() / I_hat.sum()
            result.loc[tax] = lfq
        else:
            result.loc[tax] = np.nan
